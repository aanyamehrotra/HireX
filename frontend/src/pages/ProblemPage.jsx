...
function ProblemPage() {
  ...
  useEffect(() => {
    if (problem) {
      const starterCode = problem.starterCode?.[selectedLanguage] ||
        (problem.starterCode?.javascript ? "// Language not supported for this problem" : "");
      setCode(starterCode);
      setOutput(null);
    }
  ...
  const handleRunCode = async () => {
    ...
    if (result.success) {
      // Logic for checking success might differ if we don't have expectedOutput
      // For LeetCode problems, we might not have expectedOutput available directly in the same format
      // So we'll show the output and if we have expectedOutput, compare it.
