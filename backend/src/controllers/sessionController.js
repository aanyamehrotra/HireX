...
export async function joinSession(req, res) {
  try {
    ...
    if (session.host.toString() === userId.toString()) {
      return res.status(200).json({ session, message: "Host re-joining session" });
    }

    // If the user is already the participant, just return the session
    if (session.participant?.toString() === userId.toString()) {
      return res.status(200).json({ session, message: "Participant re-joining session" });
    }
...
export async function updateSession(req, res) {
  try {
    ...
    // Update channel info if possible for visual consistency
    try {
      const channel = chatClient.channel("messaging", session.callId);
      ...
      console.log("Failed to update stream channel info:", err.message);
